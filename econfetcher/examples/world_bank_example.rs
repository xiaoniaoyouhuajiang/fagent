...
use econfetcher::world_bank::{SeriesOptions, WorldBankClient};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match client.get_countries().await {
        Ok(countries) => {
            ...
            if let Some(first_country) = countries.first() {
                println!(
                    "   First country: {} ({})",
                    first_country.name, first_country.id
                );
            }
        }
    ...
    match client.get_indicators().await {
        Ok(indicators) => {
            ...
            if let Some(first_indicator) = indicators.first() {
                println!(
                    "   First indicator: {} ({})",
                    first_indicator.name, first_indicator.id
                );
            }
        }
    ...
    match client.search_indicators("population").await {
        Ok(indicators) => {
            println!(
                "   Found {} population-related indicators",
                indicators.len()
            );
