...
fn generate_edge_fetchable_impl(
    ...
    writeln!(file, "    const ENTITY_TYPE: &'static str = \"edge_{}\";", edge_name.to_lowercase())?;
    writeln!(file, "")?;
    ...
    writeln!(file, "                Field::new(\"id\", DataType::Utf8, false),")?;
    writeln!(file, "                Field::new(\"from_node_id\", DataType::Utf8, false),")?;
    writeln!(file, "                Field::new(\"to_node_id\", DataType::Utf8, false),")?;
    writeln!(file, "                Field::new(\"from_node_type\", DataType::Utf8, false),")?;
    writeln!(file, "                Field::new(\"to_node_type\", DataType::Utf8, false),")?;
    writeln!(file, "                Field::new(\"created_at\", DataType::Timestamp(deltalake::arrow::datatypes::TimeUnit::Microsecond, Some(\"UTC\".into())), true),")?;
    writeln!(file, "                Field::new(\"updated_at\", DataType::Timestamp(deltalake::arrow::datatypes::TimeUnit::Microsecond, Some(\"UTC\".into())), true),")?;
    ...
    writeln!(file, "        let id_data: Vec<Option<String>> = data.iter().map(|item| item.id.clone()).collect();")?;
    writeln!(file, "        let from_node_id_data: Vec<Option<String>> = data.iter().map(|item| item.from_node_id.clone()).collect();")?;
    writeln!(file, "        let to_node_id_data: Vec<Option<String>> = data.iter().map(|item| item.to_node_id.clone()).collect();")?;
    writeln!(file, "        let from_node_type_data: Vec<Option<String>> = data.iter().map(|item| item.from_node_type.clone()).collect();")?;
    writeln!(file, "        let to_node_type_data: Vec<Option<String>> = data.iter().map(|item| item.to_node_type.clone()).collect();")?;
    writeln!(file, "        let created_at_data: Vec<Option<DateTime<Utc>>> = data.iter().map(|item| item.created_at.clone()).collect();")?;
    writeln!(file, "        let updated_at_data: Vec<Option<DateTime<Utc>>> = data.iter().map(|item| item.updated_at.clone()).collect();")?;
    ...
    if let Some(properties) = properties {
        for field in properties {
            ...
            writeln!(file, "        let {}_data: Vec<{}> = data.iter().map(|item| item.{}.clone()).collect();", 
    ...
    writeln!(file, "        arrays.push(crate::auto_fetchable::to_arrow_array(id_data)?);")?;
    writeln!(file, "        arrays.push(crate::auto_fetchable::to_arrow_array(from_node_id_data)?);")?;
    writeln!(file, "        arrays.push(crate::auto_fetchable::to_arrow_array(to_node_id_data)?);")?;
    writeln!(file, "        arrays.push(crate::auto_fetchable::to_arrow_array(from_node_type_data)?);")?;
    writeln!(file, "        arrays.push(crate::auto_fetchable::to_arrow_array(to_node_type_data)?);")?;
    writeln!(file, "        arrays.push(crate::auto_fetchable::to_arrow_array(created_at_data)?);")?;
    writeln!(file, "        arrays.push(crate::auto_fetchable::to_arrow_array(updated_at_data)?);")?;
