...
use fstorage::{
    ...
    schemas::generated_schemas::{Calls, HasVersion},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    storage
        .lake
        .write_edges("has_version", has_version_edges)
        .await?;
    ...
    let additional_has_version = vec![HasVersion {
    ...
    storage
        .lake
        .write_edges("has_version", additional_has_version)
        .await?;

    // Add more CALLS edges
    let additional_calls = vec![Calls {
    ...
    let out_edges = storage
        .lake
        .get_out_edges("project-rust-lang-rust", Some("has_version"))
        .await?;
    println!("📤 Out edges count: {}", out_edges.len());

    let in_edges = storage
        .lake
        .get_in_edges("version-v1.0.0", Some("has_version"))
        .await?;
    ...
    println!(
        "✅ Edge data stored in: {}/silver/edges/",
        config.lake_path.display()
    );
    println!(
        "✅ HAS_VERSION edges: {}/silver/edges/has_version/",
        config.lake_path.display()
    );
    println!(
        "✅ CALLS edges: {}/silver/edges/calls/",
        config.lake_path.display()
    );
