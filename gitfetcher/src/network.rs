...
pub async fn fetch_user_follow_network(
    ...
    while let Some((current_user_id, current_user_login, current_depth)) = queue.pop_front() {
        ...
        println!(
            "Processed user {} (depth {}): {} followers, {} following",
...
pub async fn fetch_repo_fork_network(
    ...
    // Add immediate forks
    let initial_forks = fetch_repo_forks_recursive(
        fetcher,
        owner,
        repo,
        1,
...
pub async fn fetch_all_network_data(
    ...
    let user_collaborations =
        fetch_user_collaboration_network(fetcher, owner, repo, max_items).await?;

    // Fetch fork network
    let repo_forks = fetch_repo_fork_network(fetcher, owner, repo, fork_depth, max_items).await?;

    // Fetch repository owner's follow network (limited scope)
    let user_follow_relations = if user_depth > 0 {
        fetch_user_follow_network(
            fetcher,
            &repo_info.owner.as_ref().unwrap().login,
            user_depth,
            max_items,
        )
        .await?
