...
pub fn analyze_user_follow_network(relations: &[UserFollowRelation]) -> Result<NetworkMetrics> {
    ...
    for rel in relations {
        ...
        graph.entry(&rel.follower_login)
...
pub fn analyze_repo_fork_network(forks: &[RepoFork]) -> Result<NetworkMetrics> {
    ...
    for fork in forks {
        ...
        graph.entry(&fork.source_repo_name)
            .or_insert_with(HashSet::new)
...
pub fn analyze_collaboration_network(collaborations: &[UserCollaboration]) -> Result<NetworkMetrics> {
    let mut graph = HashMap::new();
    let mut nodes = HashSet::new();
...
fn calculate_network_metrics(
    ...
    let edge_count = graph.values().map(|neighbors| neighbors.len()).sum::<usize>();
...
pub fn find_top_influential_users(relations: &[UserFollowRelation], top_n: usize) -> Vec<(String, usize)> {
    ...
    let mut sorted: Vec<_> = user_degrees.into_iter()
...
pub fn find_most_forked_repos(forks: &[RepoFork], top_n: usize) -> Vec<(String, usize)> {
    ...
    let mut sorted: Vec<_> = fork_counts.into_iter()
...
pub fn find_most_active_collaborators(collaborations: &[UserCollaboration], top_n: usize) -> Vec<(String, usize)> {
    ...
    let mut sorted: Vec<_> = activity_counts.into_iter()
...
pub fn generate_network_summary(network_data: &NetworkData) -> Result<String> {
    ...
    if !network_data.user_collaborations.is_empty() {
        ...
        let top_collaborators = find_most_active_collaborators(&network_data.user_collaborations, 5);
